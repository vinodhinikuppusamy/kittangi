...
if (!existsSync(distDir)) {
  console.error(`ERROR: ${distDir} not found. Run build-client first.`);
  process.exit(1);
}

// Auto-increment build number
mkdirSync(buildsDir, { recursive: true });
const prefix = "kittangi-frontend-build-";
...
console.log(`Packaging frontend as ${buildName}...`);

mkdirSync(buildDir, { recursive: true });
cpSync(distDir, buildDir, { recursive: true });

// Optional: copy .htaccess and web.config if present
...
if (existsSync(webConfig)) cpSync(webConfig, path.resolve(buildDir, "web.config"));
...
console.log(`Frontend packaged: ${zipPath}`);
